...
{
	gint clock_rate,ret = -1;
	char *p,*mime,*c;
	char tmp[64];

	/* parse the rtpmap */
	strncpy(tmp, rtpmap, sizeof(tmp));
...
int rtp_profile_move_payload(RtpProfile *prof,int oldpos,int newpos){
    PayloadType *pt = prof->payload[newpos];

	if (pt && (pt->flags & PAYLOAD_TYPE_ALLOCATED))
...
{
	RtpProfile *prof=g_new(RtpProfile,1);
