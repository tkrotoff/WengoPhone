...
void eXosip_set_firewallport(const char *firewall_port)
{
	...
	snprintf(eXosip.j_firewall_port,50, "%s", firewall_port);
}
			 ...
			 char *public_video_sdp_port)
{
  ...
  snprintf(jc->c_sdp_port,9, "%s", public_sdp_port ? public_sdp_port : local_audio_sdp_port);
  snprintf(jc->c_video_port,9, "%s", public_video_sdp_port ? public_video_sdp_port : local_video_sdp_port);
...
int eXosip_answer_call(int jid, int status, char *local_sdp_port, char *contact, char *local_video_port, char *public_sdp_port, char *public_video_port )
{
  int i = -1;
  eXosip_dialog_t *jd = NULL;
  eXosip_call_t *jc = NULL;
  ...
  if (tr==NULL)
	{
	  ...
	  if (tr!=NULL && tr->last_response!=NULL &&
	    ...
	    { /* answer with 603 */
	      i = eXosip_answer_call(jid, 603, 0, 0, 0, 0, 0);
	      return i;
	    }
	}
