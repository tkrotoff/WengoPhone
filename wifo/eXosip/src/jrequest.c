...
generating_request_out_of_dialog(osip_message_t **dest, char *method_name,
				 ...
				 char *proxy)
{
  return generating_request_out_of_dialog2(dest, method_name, to, transport, from, proxy, 20);
}
		    ...
		    char *proxy, char *contact, int expires, char *route, int seqnum)
{
  ...
  i = generating_request_out_of_dialog2(reg, "REGISTER", proxy, "UDP",
				       from, route, seqnum);
  if (i!=0) return -1;
				 ...
				 char *from, char *route)
{
  ...
  i = generating_request_out_of_dialog(publish, "PUBLISH", to, "UDP", from,
				       route);
  if (i!=0) return -1;
				 ...
				 char *from, char *route)
{
 ...
 osip_message_set_expires(*subscribe, SUBSCRIBE_REFRESH_INTERVAL);
...
{
  return _eXosip_build_request_within_dialog2(dest, method_name, dialog, transport, 0);
}


int
_eXosip_build_request_within_dialog2(osip_message_t **dest, char *method_name,
				   osip_dialog_t *dialog, char *transport, int seqnum)
{
  int i;
  osip_message_t *request;
#ifdef SM
