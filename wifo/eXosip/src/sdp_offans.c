...
int eXosip_get_sdp_media_info(sdp_message_t *sdp, const char *mtype, char *payload_name,  int pnsize, char *ipaddr, int ipsize, int *port)
{
  sdp_media_t *med; 
  char *payload;
  int n, pos_attr;
...
int eXosip_retrieve_sdp_negotiation_result_for(osip_negotiation_ctx_t *ctx, const char *mtype, char *pname, int pnsize)
{
  ...
  return eXosip_get_sdp_media_info(osip_negotiation_ctx_get_local_sdp(ctx), mtype, pname, pnsize, 0,0,0);
