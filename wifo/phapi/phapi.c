...
//TODO: Add a setter function to set phcfg.asyncmode
	phcfg.asyncmode = 1;
	if (!phcfg.asyncmode)
	{
		phWaitTimeout = 1;
...
MY_DLLEXPORT OWPL_RESULT owplAudioSetConfigString(const char* szAudioConfig)
{
	...
	strncpy(phcfg.audio_dev, szAudioConfig, sizeof(phcfg.audio_dev));
	return OWPL_RESULT_SUCCESS;
}
 ...
 * @param  server
 * @param  proxy
 * @param  regTimeout	if regTimeout > 0 : The newly created line will be registered right away with the SIP 
 *											server using the timeout as Expire header
 *						if regTimeout = 0 : The newly created line is not registered with Sip server. We need to
 *											call owplLineRegister later to register the line
...
{	
	...
	if (vl) {
		...
		*phLine = ph_vline2vlid(vl);
		...
		ret = phAddVline2(displayname, username, server, proxy, regTimeout);
		if (ret < 0) {
			return OWPL_RESULT_FAILURE;
		}
...
owplLineSetOpts(const OWPL_LINE hLine, LineOptEnum Opt, const void *Data)
{
	...
	case OWPL_LINE_OPT_PROXY:
		...
		{
			if (vl->LineState == OWPL_LINE_STATE_REGISTERED) {
				// Unregister it first
