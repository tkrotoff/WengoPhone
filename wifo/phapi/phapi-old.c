...
phAddVline2(const char *displayname, const char* username, const char *server, const char*  proxy,  int regTimeout)
{
	...
	vl->LineState = OWPL_LINE_STATE_NOT_ACTIVE;
...
phDelVline(int vlid, int regTimeout)
{
	...
	/* 
	if the line has an associatied phWaitTimeout, 
	...
	*/
	if (vl->regTimeout)
	{
		...
		vl->regTimeout = 0;
		//vl->used = VL_DELETING;
		vl->LineState = OWPL_LINE_STATE_DELETING;
	...
	if (vl->used != OWPL_LINE_STATE_DELETING)
	{
		return 0;
	}
...
void ph_reg_progress(eXosip_event_t *je)
{
	...
	if (je->type == EXOSIP_REGISTRATION_SUCCESS)
	{
		...
		if (vl->LineState == OWPL_LINE_STATE_DELETING)
		{
			{
				vline_free(vl);
			}
	...
	else if (je->type == EXOSIP_REGISTRATION_FAILURE)
	{
		...
		{
			owplFireLineEvent(vlid, LINESTATE_REGISTER_FAILED, LINESTATE_REGISTER_FAILED_NOT_AUTHORIZED, 0);
		}
		else {
			owplFireLineEvent(vlid, LINESTATE_REGISTER_FAILED, LINESTATE_REGISTER_FAILED_COULD_NOT_CONNECT, 0);
		}
...
ph_keep_refreshing()
{
	...
	if (eXosip_get_srv_idle_time() > MAX_SRV_IDLE_TIME) 
	{
		...
		owplFireLineEvent(0, LINESTATE_REGISTER_FAILED, LINESTATE_REGISTER_FAILED_COULD_NOT_CONNECT, 0);
	}
}
...
ph_api_thread(void *arg)
{
	...
	while(1)
	{
		...
		if (ph_event_get() == -2)
		{
			...
			owplFireLineEvent(0, LINESTATE_REGISTER_FAILED, LINESTATE_REGISTER_FAILED_NOT_AUTHORIZED, 0);
			//phTerminate();
			break;
		}
	}
