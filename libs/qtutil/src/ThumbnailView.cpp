...
const int SELECTION_CORNER_RADIUS = 18;
const int SELECTION_BORDER_SIZE = 2;
const int SELECTION_GRADIENT1 = 150;
const int SELECTION_GRADIENT2 = 100;
...
public:
	...
	virtual void paint( QPainter * painter, const QStyleOptionViewItem & option, const QModelIndex & index ) const {
		...
		if (option.state & QStyle::State_Selected) {
			...
			drawSelectionRect(painter, selectionRect, color);
		}
...
private:
	void drawSelectionRect(QPainter * painter, QRect rect, const QColor & color) const {
        ...
        gradient.setColorAt(0.0, color.light(SELECTION_GRADIENT1));
        gradient.setColorAt(1.0, color.light(SELECTION_GRADIENT2));
        painter->setBrush(gradient);
		painter->setPen(QPen(color, SELECTION_BORDER_SIZE));
