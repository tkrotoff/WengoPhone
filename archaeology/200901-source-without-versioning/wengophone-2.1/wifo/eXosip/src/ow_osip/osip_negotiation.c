...
static __payload_t *
_osip_negotiation_find_payload_by_mime (osip_list_t *codecs, char *mime,  int defltrate, int defltchans)
{
  __payload_t *my;
  int pos = 0;

  while (!osip_list_eol (codecs, pos))
    {
      my = (__payload_t *) osip_list_get (codecs, pos);
      if (my->a_rtpmap)
	{
	  char *mymime;
	  ...
	  if (*mymime)
	    {
	      if (!mimecmp(mymime, mime, defltrate, defltchans))
		return my;
	    }
	}
      pos++;
...
static __payload_t *
osip_negotiation_find_audio_payload_by_mime (osip_negotiation_t * config,
				     char *mime)
{
  return _osip_negotiation_find_payload_by_mime (config->audio_codec, mime, 8000, 1);
}



static __payload_t *
osip_negotiation_find_video_payload_by_mime (osip_negotiation_t * config,
				     char *mime)
{
  return _osip_negotiation_find_payload_by_mime (config->video_codec, mime, 90000, 1);
}
	  ...
	  do
	      ...
	      if (payload != NULL)
		{
		  ...
		  if (pnum >= 96)  /* for dynamic payload codes we prefer to match mime types */
 		    {
		      if (sdp_message_m_payload_mime_get(remote, i, atoi(payload), mime, sizeof(mime) - 1) == 0)
			my_payload = osip_negotiation_find_audio_payload_by_mime (config, mime);
		    }


		  if (my_payload == NULL)
		      my_payload = osip_negotiation_find_audio_payload (config, payload);
	  ...
	  do
	      ...
	      if (payload != NULL)
		{
		  ...
		  if (pnum >= 96)  /* for dynamic payload codes we prefer to match mime types */
 		    {
		      if (sdp_message_m_payload_mime_get(remote, i, atoi(payload), mime, sizeof(mime) - 1) == 0)
			my_payload = osip_negotiation_find_video_payload_by_mime (config, mime);
		    }


		  if (my_payload == NULL)
		      my_payload = osip_negotiation_find_video_payload (config, payload);
