  ...
  if (sdp!=NULL)
    {
      ...
      */
      ...
      if (gotpayload)
	eXosip_get_sdp_media_info(sdp, "audio", NULL, 0, je->remote_sdp_audio_ip, sizeof(je->remote_sdp_audio_ip), &je->remote_sdp_audio_port);
      else
	je->payload = eXosip_get_sdp_media_info(sdp, "audio", je->payload_name, sizeof(je->payload_name), 
					       je->remote_sdp_audio_ip, sizeof(je->remote_sdp_audio_ip), &je->remote_sdp_audio_port);

      if (gotvpayload)
	eXosip_get_sdp_media_info(sdp, "video", NULL, 0, je->remote_sdp_video_ip, sizeof(je->remote_sdp_video_ip), &je->remote_sdp_video_port);
      else
	je->video_payload = eXosip_get_sdp_media_info(sdp, "video", je->video_payload_name, sizeof(je->video_payload_name), 
					       je->remote_sdp_video_ip, sizeof(je->remote_sdp_video_ip), &je->remote_sdp_video_port);
