...
/* _send_udp:
 * This function really send the packet over the network. It chooses the right way to send the packet:
...
_send_udp(const void *buf, int len, int flags, const struct sockaddr *to, int tolen)
{
	...
	if (eXosip.use_tunnel) {
		res = eXosip.tunnel->tunnel_send(eXosip.tunnel->h_tunnel, buf, len);
	}
	else {
		eXosip_lock_jsocket();
		res = sendto (eXosip.j_socket, buf, len, flags, to, tolen);
		...
		int port, int out_socket)
{
	...
	if ((jinfo = (jinfo_t *) osip_transaction_get_your_instance(tr)))
	{
		...
		if ((!jinfo->jd || jinfo->jd->d_STATE == JD_AUTH_REQUIRED) && o_proxy && o_proxy->url && (strcmp(o_proxy->url->host, host) == 0))
		{
...
static void cb_rcv2xx_4invite(osip_transaction_t *tr,osip_message_t *sip)
{
...

  {
    ...
    int port;
    osip_message_t *ack;
    i = _eXosip_build_request_within_dialog2(&ack, "ACK", jd->d_dialog, "UDP", atoi(tr->cseq->number));
    if (i!=0) {
      jd->d_STATE = JD_ESTABLISHED;
      return ;
    }
