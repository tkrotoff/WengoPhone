...
ph_media_retrieve_decoded_frame(phastream_t *stream, ph_mediabuf_t *mbf, int clockrate)
{
  ...
  if (codedlen)
  {
	  ...
	  //Done in order to accept audio flow from the Wengo flash application 
  ...
  if (s && s->cbkInfo) 
    {
      ...
      {
		session->externalID = jc->extern_cid;
  ...
  if (sp->flags & PH_MSTREAM_FLAG_RUNNING)
  {
    ...
    if (stream->ms.payload ==  sp->ipayloads[0].number)
    {
      ...
      if (stream->ms.tunRtp)
      {
        ...
        DBG_MEDIA_ENGINE("ph_msession_audio_stream_start: Replacing audio tunnel\n");
        newTun = rtptun_connect(stream->ms.remote_ip, stream->ms.remote_port);

        if (!newTun)
        {
          DBG_MEDIA_ENGINE("ph_msession_audio_stream_start: Audio tunnel replacement failed\n");
          sp->flags |= ~PH_MSTREAM_FLAG_RUNNING;
          return -PH_NORESOURCES;
        }
