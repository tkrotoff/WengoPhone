		...
		if (userProfile.getActivePhoneLine()) {
			...
			if (userProfile.hasWengoAccount()) {
				...
				if (proxyAuthType == NetworkProxy::ProxyAuthTypeDigest) {
					...
					_qtBrowser->setUrl(std::string("http://www.wengo.com/auth/auth.php"), data, false);
				} else {
					_qtBrowser->setUrl(std::string("https://secure.wengo.com/selfcare/wengophone/auth/auth.php"), data, false);
				}
