...
QtSystray::QtSystray(QObject * parent)
	...
	SAFE_CONNECT(_trayIcon, SIGNAL(clicked( const QPoint&, int)), SLOT(showMainWindow()));
#else
...
void QtSystray::updateMenu() {
	...
	callWengoMenu->setTitle(tr("Call Wengo"));
...
void QtSystray::updateSystrayIcon() {
	...
	case EnumPresenceState::PresenceStateMulti:
		_trayIcon->setToolTip(QString("WengoPhone - ") + tr("Multiple Status"));
		break;
	case EnumPresenceState::PresenceStateUnknown:
		_trayIcon->setToolTip(QString("WengoPhone - ") + tr("Unknown Status"));
		break;
	default:
