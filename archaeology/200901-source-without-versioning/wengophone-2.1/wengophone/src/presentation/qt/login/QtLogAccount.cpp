	...
	if (!login.empty()) {
		...
		if (!_cUserProfileHandler.userProfileExists(login)) {
			...
			if (userProfile) {
				...
				{
					//wengo account
					_loginDialog->changePage(QtLoginDialog::AddWengoAccount, login);
				} else if (userProfile->hasSipAccount()) {
					//"generic" sip account
					_loginDialog->changePage(QtLoginDialog::AddSIPAccount, login);
				}
				_loginDialog->currentPage()->setBackPage(QtLoginDialog::LogAccount);
			}
		}
	}
}
...
void QtLogAccount::setInfoMessage() {
	setLoginLabel(tr("Please choose a previously used profile<br/> or fill in the informations to connect to a<br/> Wengo account"));
}
