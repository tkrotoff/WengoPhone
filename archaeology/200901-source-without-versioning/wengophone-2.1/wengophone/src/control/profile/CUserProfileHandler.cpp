...
CUserProfileHandler::CUserProfileHandler(UserProfileHandler & userProfileHandler, CWengoPhone & cWengoPhone)
	...
	_userProfileHandler.sipAccountNotValidEvent +=
		boost::bind(&CUserProfileHandler::sipAccountNotValidEventHandler, this, _1, _2);
...
void CUserProfileHandler::createAndSetUserProfile(const SipAccount & sipAccount) {
	if (sipAccount.getType() == SipAccount::SipAccountTypeWengo) {
		...
		MyThreadEvent * event =
			new MyThreadEvent(boost::bind(&CUserProfileHandler::createAndSetUserProfileWithWengoAccountThreadSafe, this, _1), wengoAccount);

		WengoPhone::getInstance().postEvent(event);

	} else {

		typedef ThreadEvent1<void (SipAccount), SipAccount> MyThreadEvent;
		MyThreadEvent * event =
			new MyThreadEvent(boost::bind(&CUserProfileHandler::createAndSetUserProfileWithSipAccountThreadSafe, this, _1), sipAccount);

		WengoPhone::getInstance().postEvent(event);
	}
}

void CUserProfileHandler::createAndSetUserProfileWithSipAccountThreadSafe(SipAccount sipAccount) {
	_userProfileHandler.createAndSetUserProfile(sipAccount);
}

void CUserProfileHandler::createAndSetUserProfileWithWengoAccountThreadSafe(WengoAccount wengoAccount) {
...
void CUserProfileHandler::sipAccountNotValidEventHandler(UserProfileHandler & sender, SipAccount & sipAccount) {
	_pUserProfileHandler->sipAccountNotValidEventHandler(sipAccount);
}
