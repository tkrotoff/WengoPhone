...
void PhApiWrapper::createSession(IMChat & imChat, const IMContactSet & imContactSet) {
...
void PhApiWrapper::init() {
	...
	*/
	...
	if(owplInit(1, _sipLocalPort, 0, 0, NULL, 0) == OWPL_RESULT_SUCCESS) {
		_isInitialized = true;
...
std::string PhApiWrapper::makeSipAddress(const std::string & contactId) {
	...
	if (contactId.compare(0, 4, "sip:") != 0) {
...
std::string PhApiWrapper::parseFromHeader(const std::string & sipFrom) {
	String uri = String(sipFrom);
	int beginOfSipUri = uri.find("<sip:") + 5;
	int endOfSipUri = uri.find(">");
	String from = uri.substr(beginOfSipUri, endOfSipUri - beginOfSipUri);
