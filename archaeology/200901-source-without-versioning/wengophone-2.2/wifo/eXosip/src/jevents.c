...
eXosip_event_add_sdp_info(eXosip_event_t *je, osip_message_t *message)
{
  ...
  osip_body_t *oldbody;
  int pos;
  int gotpayload = 0, gotvpayload = 0;
  ...
  */
  ...
  if (je->jc)
    {
      int pl = eXosip_retrieve_sdp_negotiation_audio_result(je->jc->c_ctx, je->payload_name, sizeof(je->payload_name));
      ...
      if (pl >= 0)
	{
      ...
      pl = eXosip_retrieve_sdp_negotiation_video_result(je->jc->c_ctx, je->video_payload_name, sizeof(je->video_payload_name));
  ...
  if (sdp!=NULL)
    {
      /* 
	 if we already have a payload name, we need only remote ip and port, otherwise we presume that the first payload in
	 the remote payload list will be used for the communnication
      */

	if (gotpayload)
	{
		eXosip_get_sdp_media_info(sdp, "audio", NULL, 0, je->remote_sdp_audio_ip, 
			sizeof(je->remote_sdp_audio_ip), &je->remote_sdp_audio_port, &je->ptime);
	}
	else
	{
		je->payload = eXosip_get_sdp_media_info(sdp, "audio", je->payload_name, sizeof(je->payload_name), 
			je->remote_sdp_audio_ip, sizeof(je->remote_sdp_audio_ip), &je->remote_sdp_audio_port, &je->ptime);
	}

	if (gotvpayload)
	{
		eXosip_get_sdp_media_info(sdp, "video", NULL, 0, je->remote_sdp_video_ip, 
			sizeof(je->remote_sdp_video_ip), &je->remote_sdp_video_port, 0);
	}
	else
	{
		je->video_payload = eXosip_get_sdp_media_info(sdp, "video", je->video_payload_name, 
			sizeof(je->video_payload_name), je->remote_sdp_video_ip, 
			sizeof(je->remote_sdp_video_ip), &je->remote_sdp_video_port, 0);
	}
				...
				eXosip_dialog_t *jd)
{
  ...
  {
	je->account = js->s_dialogs->account ;
  }
			     ...
			     eXosip_dialog_t *jd)
{
  ...
  {
	je->account = jn->n_dialogs->account ;
  }
...
eXosip_event_init_for_reg(int type,
			  eXosip_reg_t *jr, osip_message_t *sip)
{
  eXosip_event_t *je;
  osip_uri_param_t *expires = 0;
  osip_header_t  *mexpires = 0;
  ...
  if (sip)
	...
	osip_contact_param_get_byname(con, "expires",  &expires);
      ...
      osip_message_header_get_byname(sip, "min-expires", 0, &mexpires);
    }

  if (expires != 0 && expires->gvalue != 0)
    je->expires = atoi(expires->gvalue);
  if (mexpires != 0 && mexpires->hvalue != 0)
    je->minexpires = atoi(mexpires->hvalue);
			      ...
			      osip_message_t *sip)
{
  ...
  if (jinfo != NULL)
  {
	  je->account = jinfo->account ;
  }

  if (jinfo && jinfo->jm) {
  je->jm =  jinfo->jm;
  je->mid = jinfo->jm->m_id;
  je->jm =  jinfo->jm;
  } else {
	je->jm =  0;
	je->mid = 0;
...
eXosip_event_wait(int tv_s, int tv_ms)
{
	...
	/* basic replacement */
	{
		...
		/* <MINHPQ>: */
		if (eXosip.j_stop_ua) {
			eXosip_event_init(&je, EXOSIP_ENGINE_STOPPED);
			return je;
		}
