...
static int generating_request_out_of_dialog2(osip_message_t **dest,
					    char *method_name, char *to,
					    char *from,
					    char *proxy, int cseq);
...
generating_request_out_of_dialog(osip_message_t **dest, char *method_name,
				 ...
				 char *proxy)
{
  return generating_request_out_of_dialog2(dest, method_name, to, from, proxy, 20);
}

static int
generating_request_out_of_dialog2(osip_message_t **dest, char *method_name,
				 char *to, char *from,
				 char *proxy, int seqnum)
{
  /* Section 8.1:
  ...
  */
	...
	char locip[50];
	int doing_register;
	OWSIPAccount account = owsip_account_get_from_uri_string (from) ;
	if (account <= 0)
	{
		return -1 ;
	}
  ...
  {
	  owsip_message_set_header(request, "Event", "presence");
...
generating_register(osip_message_t **reg, char *from,
		    char *proxy, char *contact, int expires, char *route, int seqnum)
{
  int i;
  OWSIPAccount account = owsip_account_get_from_uri_string (from) ;
  if (account <= 0)
  ...
  i = generating_request_out_of_dialog2(reg, "REGISTER", proxy, from, route, seqnum);
  if (i!=0) return -1;
...

  {
    ...
    owsip_message_set_expires(*reg, exp);
  }
				 ...
				 char *route)
{
  ...
  i = generating_request_out_of_dialog(options, "OPTIONS", to, from, route);
  if (i!=0) return -1;

  /* after this delay, we should send a CANCEL */
  owsip_message_set_expires(*options, "120");
...
//<MINHPQ>
int generating_initial_publish(osip_message_t **publish, char *to,
				 char *from, char *route)
{
  ...
  i = generating_request_out_of_dialog(publish, "PUBLISH", to, from, route);
  if (i!=0) return -1;
				 ...
				 char *from, char *route)
{
  ...
  i = generating_request_out_of_dialog(subscribe, "SUBSCRIBE", to, from, route);
  if (i!=0) return -1;
  
  owsip_message_set_expires(*subscribe, eXosip.subscribe_timeout);
		       ...
		       char *route, char *buff, char *mime)
{
  ...
  i = generating_request_out_of_dialog(message, "MESSAGE", to, from, route);
  if (i!=0) return -1;
  
  owsip_message_set_expires(*message, "120");
...
{
  int i;
  i = generating_request_out_of_dialog(options, "OPTIONS", to, from, proxy);
  if (i!=0) return -1;
...
{
  int i;
  i = generating_request_out_of_dialog(info, "INFO", to, from, proxy);
  if (i!=0) return -1;
  return 0;
}
...
{
  return _eXosip_build_request_within_dialog2(dest, method_name, dialog, 0);
}


int
_eXosip_build_request_within_dialog2(osip_message_t **dest, char *method_name,
				   osip_dialog_t *dialog, int seqnum)
{
  int i;
  osip_message_t *request;
  eXosip_dialog_t * jdialog ;
  OWSIPAccount account ;

  jdialog = owsip_dialog_get (dialog) ;
  if (jdialog == NULL)
  {
	  return -1 ;
  }
  account = owsip_dialog_account_get (jdialog) ;
  if (account <= 0)
  {
	  return -1 ;
  }
...
{
  int i;
  i = _eXosip_build_request_within_dialog(bye, "BYE", dialog);
  if (i!=0) return -1;
...
{
  int i;
  i = generating_request_out_of_dialog(refer, "REFER", to, from, proxy);
  if (i!=0) return -1;

  owsip_message_set_header(*refer, "Refer-to", refer_to);
  return 0;
}
...
{
  int i;
  i = _eXosip_build_request_within_dialog(refer, "REFER", dialog);
  if (i!=0) return -1;
...
{
  int i;
  i = _eXosip_build_request_within_dialog(options, "OPTIONS", dialog);
  if (i!=0) return -1;
...
{
  int i;
  i = _eXosip_build_request_within_dialog(info, "INFO", dialog);
  if (i!=0) return -1;
  return 0;
}
...
{
  int i;
  i = _eXosip_build_request_within_dialog(ack, "ACK", dialog);
  if (i!=0) return -1;
