...
std::string EnumPresenceState::toString(PresenceState presenceState) {
	std::string result;
	...
	case PresenceStateUnavailable:
		...
		break;
	default:
		LOG_FATAL("unknown state=" + QString::number(presenceState).toStdString());
	}

	return result;
}

EnumPresenceState::PresenceState EnumPresenceState::fromString(const std::string & presenceState) {
