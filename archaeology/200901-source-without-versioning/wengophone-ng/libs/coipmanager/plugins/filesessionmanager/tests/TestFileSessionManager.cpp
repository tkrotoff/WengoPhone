...
TestFileSessionManager::TestFileSessionManager(UserProfile & userProfile)
	: IFileSessionManager Plugin(userProfile) {
}

TestFileSessionManager::~TestFileSessionManager() {
}

ISendFileSession * TestFileSessionManager::createISendFileSessionPlugin() {
	return new TestSendFileSession();
}

bool TestFileSessionManager::isProtocolSupported(EnumIMProtocol::IMProtocol accountType) const {
	if (accountType == EnumIMProtocol::IMProtocolMSN) {
		return true;
	} else {
		return false;
	}
