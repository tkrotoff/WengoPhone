...
PhApiForWengoConnect::PhApiForWengoConnect(CoIpManager & coIpManager, const Account & account)
	...
	addState(EnumConnectionState::ConnectionStateConnecting, new PhApiForWengoConnectingState(*this, *sipWrapper));
...
PhApiForWengoConnect::~PhApiForWengoConnect() {
}

bool PhApiForWengoConnect::checkValidity() {
/*	WengoAccount * wengoAccount = (WengoAccount *) _account.getPrivateAccount();

	if (!wengoAccount->isSSORequestMade()) {
		if (!detectConnectionConfigurationForSSO(false)) {
			return false;
		}

		return (ssoRequest() == SSORequest::SSORequestErrorOk);
	} else {
		return true;
	}
*/
