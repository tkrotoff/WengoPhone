...
#ifndef OWSETTINGSTESTSUITE_H
#define OWSETTINGSTESTSUITE_H
...
public:
	...
	void testSettings() {
		...
		std::string ret2 = settings.get(key2, value3);
		BOOST_CHECK(ret2 == value2);
		
		std::string ret3 = settings.get("invalid key", value3);
	...
	void testAllKeys() {
		...
		for (register unsigned i = 0 ; i < keyList.size() ; i++) {
			settings.set(keyList[i], std::string("key test"));
		}
...
public:
	...
	SettingsTestSuite() : test_suite("SettingsTestSuite") {
		...
		test_case * testRemove = BOOST_CLASS_TEST_CASE(&SettingsTest::testRemove, instance);
