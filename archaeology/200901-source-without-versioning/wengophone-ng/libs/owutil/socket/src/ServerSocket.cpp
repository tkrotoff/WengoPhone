...
bool ServerSocket::writeToClient(const std::string & connectionId, const std::string & data) {
	...
	writeStatusSignal(connectionId, error);
...
void ServerSocket::run() {
	...
	while (_started) {
		...
		if (err && FD_ISSET(_mainSock, &rfds)) {
			...
			if (sock > 0) {
				...
				connectionSignal(String::fromNumber((int)sock));
			}
		}
		else if (err) {
			SockListIterator it;
			for (it = _clientSockList.begin(); it != _clientSockList.end(); it++) {
				if (FD_ISSET(*it, &rfds)) {
					if (getRequest(*it, buff, sizeof(buff)) == -1) {
						...
						incomingRequestSignal(String::fromNumber((int) *it), std::string(buff));
					}
					break;
				}
			}
	...
	serverStatusSignal(UnknownError);
}
