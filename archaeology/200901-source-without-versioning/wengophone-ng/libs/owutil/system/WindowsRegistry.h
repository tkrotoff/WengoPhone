 ...
 */
class WindowsRegistry : NonCopyable {
public:
	...
	/**
	 * Deletes an entry to a registry key.
	 *
	 * @param rootKey the root key of type HKEY (HKEY_CLASSES_ROOT, HKEY_CURRENT_USER, ...).
	 ...
	 * @return true if success otherwise false.
	 */
	OWSYSTEM_API static bool deleteEntry(HKEY rootKey, const std::string & subkey, const std::string & entry);
};
