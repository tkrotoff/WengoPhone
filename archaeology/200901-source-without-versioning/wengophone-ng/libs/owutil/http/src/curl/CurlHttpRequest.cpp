...
int CurlHttpRequest::sendRequest(bool sslProtocol, const std::string & hostname, unsigned hostPort,
...
void CurlHttpRequest::run() {
	...
	if (_curl) {
		...
		LOG_DEBUG("http answer received");
		if (res != CURLE_OK) {
			LOG_WARN(std::string(curl_easy_strerror(res)));
			answerReceivedSignal(_lastRequestId, String::null, getReturnCode(res));
		} else {
			answerReceivedSignal(_lastRequestId, entireResponse, IHttpRequest::NoError);
		}
...
void CurlHttpRequest::setProxyUserParam() {
	if (useProxyAuthentication() && _proxyAuthentication) {
		...
		std::string proxyUserIDPassword = HttpRequest::getProxyUsername() + ":" + HttpRequest::getProxyPassword();
...
void CurlHttpRequest::setProxyParam() {
	if (useProxy()) {
		std::string proxy = HttpRequest::getProxyHost();
...
long CurlHttpRequest::getProxyAuthenticationType() {
	...
	long AuthMask = 0;
	int ret;
	std::string testUrl = "www.wengo.fr";
	...
	if (useProxy()) {
		...
		curl_easy_setopt(curl_tmp, CURLOPT_VERBOSE, verbose);
...
int curlHTTPProgress(void * curlHttpRequestInstance, double dltotal, double dlnow, double ultotal, double ulnow) {
	if (curlHttpRequestInstance) {
		...
		long requestId;
		curl_easy_getinfo(instance->_curl, CURLINFO_PRIVATE, &requestId);
		instance->emitDataReadProgressSignal(requestId, dlnow, dltotal);
		instance->emitDataSendProgressSignal(requestId, ulnow, ultotal);

		//Launches answerReceivedSignal only if the entire response content has been received
		if ((dlnow >= dltotal) && (dlnow != 0 && dltotal != 0)) {
			//instance->answerReceivedSignal(requestId, instance->entireResponse, HttpRequest::NoError);
		}
...
void CurlHttpRequest::emitDataReadProgressSignal(int requestId, double bytesDone, double bytesTotal) {
	dataReadProgressSignal(requestId, bytesDone, bytesTotal);
}

void CurlHttpRequest::emitDataSendProgressSignal(int requestId, double bytesDone, double bytesTotal) {
	dataSendProgressSignal(requestId, bytesDone, bytesTotal);
}
