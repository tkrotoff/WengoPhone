 ...
 * </pre>
 *
 * We don't use QtNetwork/QHttp and HttpRequest was developed because phApi/netlib/httptunnel use cURL
 *
 * @author Tanguy Krotoff
 */
class OWHTTP_API IHttpRequest : public QObject, Interface {
	Q_OBJECT
public:
...
Q_SIGNALS:
	 ...
	 */
	void answerReceivedSignal(int requestId, std::string answer, IHttpRequest::Error error);
	 ...
	 */
	void dataReadProgressSignal(int requestId, double bytesDone, double bytesTotal);
	 ...
	 * @param bytesTotal total size of the HTTP transfer, if 0 then the total number of bytes is not known
	 */
	void dataSendProgressSignal(int requestId, double bytesDone, double bytesTotal);
};
