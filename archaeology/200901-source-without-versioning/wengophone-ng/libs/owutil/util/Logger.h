...
#define LOG_FATAL(message) Logger::logger.fatal(std::string(__FUNCTION__), message, std::string(__FILE__), QString::number(__LINE__).toStdString())
 ...
 */
class Logger : public QObject, NonCopyable {
	Q_OBJECT
public:
...
Q_SIGNALS:
	 ...
	 */
	void messageAddedSignal(std::string message);
