...
string Path::getApplicationDirPath() {
	...
	if (ret == -1 || (unsigned) ret >= sizeof(buff)) {
		return String::null;
	}
...
std::string Path::getBundlePathFromId(const std::string & bundleId) {
	std::string result;
	CFStringRef strref = CFStringCreateWithCString(kCFAllocatorDefault, bundleId.c_str(), kCFStringEncodingASCII);
	CFBundleRef bundle = CFBundleGetBundleWithIdentifier(strref);
