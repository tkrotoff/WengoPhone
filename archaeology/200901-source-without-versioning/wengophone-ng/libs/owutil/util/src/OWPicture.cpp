...
OWPicture OWPicture::pictureFromData(const QByteArray & byteArray) {
	OWPicture result;

	result._pictureData = std::string(byteArray.constData(), byteArray.length());
...
QByteArray OWPicture::getData() const {
	return QByteArray(_pictureData.c_str(), _pictureData.size());
}
