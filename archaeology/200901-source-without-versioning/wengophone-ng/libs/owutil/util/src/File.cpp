...
std::string File::normalize(const std::string & path, bool forceDir) {
	std::string result = convertPathSeparators(path);

	if (forceDir && !String(result).endsWith(getPathSeparator())) {
		result += getPathSeparator();
	}

	if (isDirectory(result) && !String(result).endsWith(getPathSeparator())) {
		result += getPathSeparator();
	}
...
File File::createTemporaryFile() {
	...
	if (result == 0 || result > sizeof(tempPath)) {
		LOG_FATAL("GetTempPath failed");
	}

	char tempName[MAX_PATH];
	result = GetTempFileName(tempPath, "ow", 0, tempName);
	if (result == 0) {
...
std::string FileReader::read() {
	static const unsigned BUFFER_SIZE = 2000;
...
bool FileWriter::write(const std::string & data) {
	...
	bool ret = false;
	if (!isOpen()) {
		//Try to open it again if not already done
		ret = open();
	}
