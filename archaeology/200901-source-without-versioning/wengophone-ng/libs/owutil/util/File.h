...
public:
	...
	*/
	...
	/** 
	 * Normalizes the filename: convertPathSeparators is called on the given string
	 * and a PathSeparator is added at end if the file is a directory.
	 * @param forceDir if true, no autodetection will be made and path will be considered
	 * a directory. Thus a PathSeparator will be appended.
	 */
	OWUTIL_API static std::string normalize(const std::string & path, bool forceDir = false);
...
public:
	 ...
	 * @return true if data where written to a file; false otherwise
	 */
	OWUTIL_API bool write(const std::string & data);
