 ...
 */
static snd_mixer_t *
open_mixer(const char *sound_card)
{
 ...
 */
static int
close_mixer(snd_mixer_t *mixer_handle)
{
...
find_mixer_element(snd_mixer_t *mixer_handle, const char *element_name)
{
 ...
 */
static int
set_mixer_level_private(snd_mixer_t *mixer_handle, const char *name,
                        mixer_element_type type, int level)
{
  int err;
  long volume_min;
  long volume_max;
  ...
  switch (type)
  {
    ...
    case CAPTURE:
      ...
      {
        snd_mixer_selem_get_capture_volume_range(mixer_element,
          &volume_min, &volume_max);
        new_level = (((volume_max- volume_min) * level) / 100) + volume_min;
 ...
 */
static int
get_mixer_level_private(snd_mixer_t *mixer_handle, const char *name,
  ...
  switch (type)
  {
    case PLAYBACK:
      if (snd_mixer_selem_has_playback_volume(mixer_element))
      {
        snd_mixer_selem_get_playback_volume_range(mixer_element,
          &volume_min, &volume_max);
        err = snd_mixer_selem_get_playback_volume(mixer_element,
    ...
    case CAPTURE:
      if (snd_mixer_selem_has_capture_volume(mixer_element))
      {
        snd_mixer_selem_get_capture_volume_range(mixer_element,
          &volume_min, &volume_max);
        err = snd_mixer_selem_get_capture_volume(mixer_element,
          SND_MIXER_SCHN_MONO, &level);
        if (err < 0)
        {
          LOG_WARN("failed to get" + String(name) + " volume: " +String(snd_strerror(err)));
