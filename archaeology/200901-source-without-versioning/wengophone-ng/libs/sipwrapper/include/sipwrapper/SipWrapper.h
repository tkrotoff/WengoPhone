...
class AudioDevice;
class SipChatWrapper;
class SipContactPresenceWrapper;
class WebcamVideoFrame;

/**
 * Wrapper for SIP stacks.
 *
 * A SIP stack is a piece of code that implements the SIP accountType
 ...
 */
class SIPWRAPPER_API SipWrapper : public QObject, Interface {
	Q_OBJECT
public:
	...
	SipChatWrapper * getSipChatWrapper() const;

	SipContactPresenceWrapper * getSipContactPresenceWrapper() const;
	 ...
	 * @return true if making a register succeeds; false otherwise
	 */
	virtual bool registerVirtualLine(int lineId) = 0;
	...
	/**
	 * Publish the user Presence.
	 *
	 * @param lineId the id of the line on which SipWrapper must change the presence state.
	 * @param online true if 'online' must be published, false for 'offline'
	 * @param notes used to publish a personnalized message when online is true.
	 */
	virtual void publishPresence(int lineId, bool online, const std::string & note) = 0;
	...
	/**
	 * Sets HTTP proxy parameters.
	 */
	virtual void setHTTPProxy(const std::string & address, unsigned port,
...
Q_SIGNALS:
	 ...
	 */
	void phoneCallStateChangedSignal(int callId, EnumPhoneCallState::PhoneCallState state, std::string from);
	 ...
	 */
	void phoneLineStateChangedSignal(int lineId, EnumPhoneLineState::PhoneLineState state);
	 ...
	 */
	void videoFrameReceivedSignal(int callId, piximage * remoteVideoFrame, piximage * localVideoFrame);

protected:

	SipChatWrapper *_sipChatWrapper;

	SipContactPresenceWrapper *_sipContactPresenceWrapper;
