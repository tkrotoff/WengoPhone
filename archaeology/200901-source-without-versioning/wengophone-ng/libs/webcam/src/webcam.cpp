...
// WebcamFrameCaptured implementation
WebcamFrameCaptured * WebcamFrameCaptured::_instance = NULL;

WebcamFrameCaptured::WebcamFrameCaptured() {
}

WebcamFrameCaptured * WebcamFrameCaptured::getInstance() {
	if (!_instance) {
		_instance = new WebcamFrameCaptured();
	}

	return _instance;
}

void WebcamFrameCaptured::webcamFrameCapturedSlot(piximage * image) {
...
webcam * webcam_get_instance() {
	...
	SAFE_CONNECT_RECEIVER_TYPE(wc->driver, SIGNAL(frameCapturedSignal(piximage *)),
		WebcamFrameCaptured::getInstance(), SLOT(webcamFrameCapturedSlot(piximage *)), Qt::DirectConnection);
