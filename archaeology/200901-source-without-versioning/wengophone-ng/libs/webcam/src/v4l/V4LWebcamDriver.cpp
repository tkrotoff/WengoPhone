...
std::string V4LWebcamDriver::getDefaultDevice() {
...
WebcamErrorCode V4LWebcamDriver::setPalette(pixosi palette) {
	...
	for (int i = 0; try_palettes[i] != 0; i++) {
...
void V4LWebcamDriver::run() {
	piximage * image = pix_alloc(getPalette(), getWidth(), getHeight());

	while (isOpen() && !_terminate) {
		msleep(1000 / _fps);

		int fsize = pix_size(image->palette, image->width, image->height);
		int len = read(_fhandle, image->data, pix_size(image->palette, image->width, image->height));
...
V4LWebcamDriver::DevNameArray V4LWebcamDriver::getDevices2_6() {
	const std::string dir = "/sys/class/video4linux";
	DevNameArray array;
	File sysDir(dir);
	...
	for (register unsigned i = 0 ; i < list.size() ; i++) {
		if (list[i][0] != '.') {
			std::ifstream nameFile((dir + "/" + list[i] + "/name").c_str());
			char buffer[512];
...
V4LWebcamDriver::DevNameArray V4LWebcamDriver::getDevices2_4() {
	const std::string dir = "/proc/video/dev";
	DevNameArray array;
	File sysDir(dir);
	StringList list = sysDir.getDirectoryList();
