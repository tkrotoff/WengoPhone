...
StringList DirectXWebcamDriver::getDeviceList() {
	...
	while (true) {
		...
		if (((std::string) _bstr_t(DevicePath)).find("pci") == std::string::npos) {
			deviceName = (std::string) _bstr_t(FriendlyName);
		...
		if (hr == S_OK) {
			...
			if (hr != S_OK) {
				...
				LOG_DEBUG("assigned caps : ("
					+ QString::number(palette).toStdString()
					+ "," + QString::number(fps).toStdString()
					+ "," + QString::number(resolutionWidth).toStdString()
					+ "," + QString::number(resolutionHeight).toStdString()
					+ ")");
				break;
			}
		}
