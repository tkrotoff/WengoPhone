...
void frame_captured(webcam * sender, piximage * image, void * userdata) {
	...
	std::cout << "frame_captured" << std::endl;
}
...
WebcamTest::WebcamTest()
	...
	std::string device = webcam_get_default_device(_webcam);
	...
	std::cout << "Using width: " << webcam_get_width(_webcam)
		<< ", height: " << webcam_get_height(_webcam)
		<< " FPS: " << webcam_get_fps(_webcam) << std::endl;
	...
	if (webcam_is_open(_webcam)) {
		...
		if (!_image) {
			QMessageBox::critical(this, "Webcam test", "error while creating _image");
		}
		std::cout << "**Starting capture..." << std::endl;
		webcam_start_capture(_webcam);
	} else {
		QMessageBox::critical(this, "Webcam test", "No webcam found");
	}
}
