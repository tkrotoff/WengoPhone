...
parse_account(xmlnode *node)
{
	...
	if ((child != NULL) && ((data = xmlnode_get_data(child)) != NULL))
	{
		// WENGO : see purple_account_set_alias() definition
		if (*data != '\0')
			purple_account_set_alias(ret, data, FALSE);
		g_free(data);
	}
...
// WENGO : add an third parameter to manage network alias
void
						 ...
						 gboolean remote_update)
{
			...
			g_utf8_collate(account->alias, alias))
	{
		...
		/* WENGO */
		if (remote_update && account->alias != NULL)
		{
			PurplePluginProtocolInfo *prpl_info = NULL;
			...
			if (prpl_info != NULL && prpl_info->set_alias != NULL)
				prpl_info->set_alias(gc, account->alias);
		}
		/* ****** */
