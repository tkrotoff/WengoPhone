...
eXosip_notify_init(eXosip_notify_t **jn, OWSIPAccount account, osip_message_t *inc_subscribe)
{
  ...
  *jn = (eXosip_notify_t *)osip_malloc(sizeof(eXosip_notify_t));
  ...
  osip_free(uri);
	...
	(*jn)->n_contact_info = osip_malloc (200) ;
	if ((*jn)->n_contact_info == NULL)
	{
		return -1 ;
	}
	(*jn)->n_contact_info = owsip_account_contact_get (account, (*jn)->n_contact_info, 200) ;
	if ((*jn)->n_contact_info == NULL)
	{
		return -1 ;
	}
	/* contact begins after '<' */
	(*jn)->n_contact_info = strchr ((*jn)->n_contact_info, '<') ;
	if ((*jn)->n_contact_info == NULL)
	{
		return -1 ;
	}
	(*jn)->n_contact_info ++ ;
	/* contact ends before '>' */
	contact_end = strchr ((*jn)->n_contact_info, '>') ;
	if (contact_end == NULL)
	{
		return -1 ;
	}
