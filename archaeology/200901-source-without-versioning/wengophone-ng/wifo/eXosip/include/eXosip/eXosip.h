...
typedef enum OWSIPDirection
{
	OWSIP_IN,
	OWSIP_OUT
} OWSIPDirection ;

typedef int OWSIPAccount ;
OWSIPAccount owsip_account_new
(
	const char * name,
	const char * user,
	const char * domain,
...
int owsip_account_free (OWSIPAccount account) ;
/* TODO: remove these functions (they should be private) */
...
char * owsip_account_contact_get (OWSIPAccount account, char * contact, size_t contact_size) ;
int
...
/* end of private functions to move */
typedef int (* OWSIPAccountMessageFilter)
(
...
int owsip_account_message_filter_set
(
	OWSIPAccount account,
...
/**
 * Initialize the eXtented oSIP library.
 * 
 * @param input    command input (RESERVED FOR FUTUR USE).
 * @param output   resulting output (RESERVED FOR FUTUR USE).
 * @param udp_port     the udp port the softphone will use to create a listening socket
 * @param tcp_port     the tcp port the softphone will use to create a listening socket
 * @param tls_port     the tls port the softphone will use to create a listening socket
 */
int eXosip_init(FILE *input, FILE *output, int udp_port, int tcp_port, int tls_port);

/**
 * Reinitialize the network part of the eXtented oSIP library.
 * 
 * @param udp_port     the udp port the softphone will use to create a listening socket
 * @param tcp_port     the tcp port the softphone will use to create a listening socket
 * @param tls_port     the tls port the softphone will use to create a listening socket
...
/**
 * Force eXosip to use a specific ip address in all its SDP messages.
 *
 * @param mip 	the ip address.
 *
 * If set to NULL, then the media ip address will be the same as local ip address
 */
...
void eXosip_set_firewallport(const char *firewall_port);
...
/**
 * Remove a video payload.
 * 
 */
void eXosip_sdp_negotiation_remove_video_payloads(void);
...
/**
 * Lock eXosip.j_socket. 
 * We need to lock the socket in every function that uses it because 
 * eXosip_ping_proxy will modify the eXosip.j_socket with a call to setsockopt.
 */
int
...
/**
 * Send and Out of Dialog OPTIONS message
 * 
 * @param to        SIP url for callee.
 * @param from      SIP url for caller.
 * @param route     Route header
 */
int   eXosip_options(OWSIPAccount account, char *from, char *to, char *route);
 ...
 */
int   eXosip_initiate_call    (OWSIPAccount account, osip_message_t *invite, void *reference,
			       void *sdp_context_reference, char *local_sdp_port, char *local_sdp_video_port, char *public_sdp_port, char *public_video_port);
 ...
 * @param body 			  The body.
 */
int   eXosip_initiate_call_with_body    (OWSIPAccount account, osip_message_t *invite, const char *body_type, const char *body, void *reference);
 ...
 * @param pnsize       size of the payload_name buffer
 */
int   eXosip_retrieve_negotiated_audio_payload(int jid, int *payload, char *payload_name, int pnsize);



/**
 * Retreive video payload negotiated for this dialog
 * 
 * @param jid          dialog id of call.
 * @param payload      will hold the negotited payload code
 * @param payload_name will hold negotioated payload mime string
 * @param pnsize       size of the payload_name buffer
 */
int eXosip_retrieve_negotiated_video_payload(int jid, int *payload, char *payload_name, int pnsize);

/**
 * Retreive specific payload accepted for this dialog
 * 
 * @param jid          dialog id of call.
 * @param payload_name holds negotioated payload mime string
 * @param pnsize       size of the payload_name buffer
 */
...
/**
 * Tranfer a call to a give destination call
 * 
 * @param jid          dialog id of call.
 ...
 * @param route     value for the Route header
 */
int   eXosip_register_init (OWSIPAccount account, char *from, char *proxy, char *route);
 ...
 */
int  eXosip_message    (OWSIPAccount account, char *to, char *from, char *route, 
						char *buff, char *mime);


/**
 * Retry sending the instant message (possibly adding auth info)
 * 
 * @param mid return  value of the previois eXosip_mesage call
 */
 ...
 * @param   winfo   0 if subscribe is for presence event
 *                   1 if subscribe is for watcher info (presence.winfo) event
*/
#define SUBSCRIBE_REFRESH_INTERVAL "600"
...
typedef enum eXosip_event_type_t {
  ...
  EXOSIP_CALL_REFER_FAILURE,         /* announce error in executing refer  */

  EXOSIP_CALL_REPLACES,              /*  incoming INVITE with Repalces header */
  EXOSIP_ENGINE_STOPPED,
 ...
 *  -  Static buffer is not good! (what if we received a SDP whose length is greater than 1000? )
 *  -  It should be a buffer to contain the payload of the sip message, not only SDP for INVITE, hence it should be renamed
 */
/*  char                sdp_body[1000]; */
  char *			  msg_body;
  int                 online_status;
  ...
  int                 replacedcid;  /* CID of the replaced call */
  ...
  int expires;        /* expiration limit for registration */
  int minexpires;
