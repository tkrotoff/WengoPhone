...
unsigned short SipAccount::_testSIPRetry = 3;
...
bool SipAccount::discoverForSIP() {
	...
	if (!config.getNetWorkTunnelNeeded()) {
		...
		for (unsigned short i = 0; i < _testSIPRetry; i++) {
			...
			if (_networkDiscovery.testSIP(_sipProxyServerHostname, (unsigned short) _sipProxyServerPort, (unsigned short) _localSIPPort)) {
