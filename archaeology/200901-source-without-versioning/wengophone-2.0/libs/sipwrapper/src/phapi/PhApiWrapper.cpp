...
void PhApiWrapper::subscribeToPresenceOf(const std::string & contactId) {
	...
	if ((sid = phLineSubscribe(_wengoVline, sipAddress.c_str(), 0)) > 0) {
		_contactSubscribeIdMap[contactId] = sid;
...
void PhApiWrapper::unsubscribeToPresenceOf(const std::string & contactId) {
	std::map<const std::string, int>::iterator it =_contactSubscribeIdMap.find(contactId);

	if (it != _contactSubscribeIdMap.end()) {
		phUnsubscribe(_contactSubscribeIdMap[contactId]);
		_contactSubscribeIdMap.erase(it);
	}
}
