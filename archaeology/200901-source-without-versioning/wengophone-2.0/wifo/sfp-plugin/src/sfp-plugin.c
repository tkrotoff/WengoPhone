...
SFP_PLUGIN_EXPORTS int sfp_receive_file(int cid, const char * filename){
	...
	if(pthread_create(&thread, NULL, (void *)sfp_transfer_receive_file, (void *)session) != 0){
		...
		if(sfp_cbks != NULL && sfp_cbks->receivingFileBegin) sfp_cbks->receivingFileBegin(cid, session->remote_username, session->local_filename, session->file_type, session->file_size);	
	}
...
SFP_PLUGIN_EXPORTS int sfp_cancel_transfer(int call_id){
	...
	if(session->isInitiated(session)) {
		...
		if(session->isCancelled(session)) {
			...
			// notify the GUI
			if(sfp_cbks != NULL && sfp_cbks->transferCancelled) sfp_cbks->transferCancelled(call_id, session->local_filename, session->file_type, session->file_size);
...
SFP_PLUGIN_EXPORTS int sfp_pause_transfer(int call_id){
	...
	if(( !session->isPaused(session) || !session->isPausedByPeer(session) ) && phHoldOn(call_id, "application/sfp") == TRUE){
		session->updateState(session, SFP_ACTION_PAUSE);
		if(session->isPaused(session)) {
			if(sfp_cbks != NULL && sfp_cbks->transferPaused) { sfp_cbks->transferPaused(call_id, session->remote_username, session->local_filename, session->file_type, session->file_size); }
...
SFP_PLUGIN_EXPORTS int sfp_resume_transfer(int call_id){
	...
	g_mutex_lock(pause_mutex);
	if(session->isPaused(session) && phHoldOff(call_id, "application/sfp") == TRUE){
		session->updateState(session, SFP_ACTION_RESUME);
		if(session->isRunning(session)) {
			if(sfp_cbks != NULL && sfp_cbks->transferResumed) { sfp_cbks->transferResumed(call_id, session->remote_username, session->local_filename, session->file_type, session->file_size); }
			g_mutex_unlock(pause_mutex);
			return TRUE;
		}
	}
...
static void sfp_receive_terminaison(sfp_session_info_t * session, sfp_returncode_t code) {
	...
	if(code != SUCCESS) {
		if(sfp_cbks != NULL && sfp_cbks->transferFromPeerFailed) sfp_cbks->transferFromPeerFailed(call_id, session->remote_username, session->local_filename, session->file_type, session->file_size);
		// MAYBE send a BYE?
		...
		if(session->isCancelled(session)) {
			...
			if(sfp_cbks != NULL && sfp_cbks->transferCancelledByPeer) sfp_cbks->transferCancelledByPeer(call_id, session->remote_username, session->local_filename, session->file_type, session->file_size);
			remove(session->filename);
		} else if(session->isFinished(session)) {
			if(sfp_cbks != NULL && sfp_cbks->transferFromPeerFinished) sfp_cbks->transferFromPeerFinished(call_id, session->remote_username, session->local_filename, session->file_type, session->file_size);
		} else {
			if(sfp_cbks != NULL && sfp_cbks->transferFromPeerFailed) sfp_cbks->transferFromPeerFailed(call_id, session->remote_username, session->local_filename, session->file_type, session->file_size);
			// MAYBE send a BYE?
			remove(session->filename);
...
static void sfp_send_terminaison(sfp_session_info_t * session, sfp_returncode_t code){
	...
	if(code != SUCCESS) {
		...
		if(session->isCancelled(session)) {
			...
			if(sfp_cbks != NULL && sfp_cbks->transferToPeerFailed) sfp_cbks->transferToPeerFailed(call_id, session->remote_username, session->local_filename, session->file_type, session->file_size);
			phBye(call_id);
		}
	}
...
static void sfp_on_EXOSIP_CALL_NEW(eXosip_event_t * event){
	...
	// TODO notify GUI
	if(sfp_cbks != NULL && sfp_cbks->newIncomingFile) sfp_cbks->newIncomingFile(call_id, session->remote_username, session->local_filename, session->file_type, session->file_size);
}
/**
...
static void sfp_on_EXOSIP_CALL_ANSWERED(eXosip_event_t * event){
	...
	if(session->isTrying(session)){ // INVITE has been sent, waiting for a 200 OK to start the transfer
		...
		if(pthread_create(&thread, NULL, (void *)sfp_transfer_send_file, (void *)session) != 0){
			...
			if(sfp_cbks != NULL && sfp_cbks->transferToPeerFailed) sfp_cbks->transferToPeerFailed(event->cid, session->remote_username, session->local_filename, session->file_type, session->file_size);
			sfp_remove_session_info(event->cid);

			return;
		}else{
			if(sfp_cbks != NULL && sfp_cbks->sendingFileBegin) sfp_cbks->sendingFileBegin(event->cid, session->remote_username, session->local_filename, session->file_type, session->file_size);	
		}
	}
}
...
static void sfp_on_EXOSIP_CALL_REQUESTFAILURE(eXosip_event_t * event){
	...
	if(event->status_code == 486) {
		if(sfp_cbks != NULL && sfp_cbks->transferCancelledByPeer) sfp_cbks->transferCancelledByPeer(event->cid, event->req_uri, session->local_filename, session->file_type, session->file_size);
	} else {
		if(sfp_cbks != NULL && sfp_cbks->transferToPeerFailed) sfp_cbks->transferToPeerFailed(event->cid, event->req_uri, session->local_filename, session->file_type, session->file_size);
	}
...
static void sfp_on_EXOSIP_CALL_CLOSED(eXosip_event_t * event){ // BYE received
	...
	if(session->isInitiated(session)) {
		...
		if(session->isCancelledByPeer(session)) {
			if(sfp_cbks != NULL && sfp_cbks->transferCancelledByPeer) sfp_cbks->transferCancelledByPeer(event->cid, session->remote_username, session->local_filename, session->file_type, session->file_size);
			sfp_remove_session_info(event->cid);
...
static void sfp_on_EXOSIP_CALL_HOLD(eXosip_event_t * event){
	...
	if(!session->isPaused(session) || !session->isPausedByPeer(session)) {
		...
		if(session->isPausedByPeer(session)) {
			if(sfp_cbks != NULL && sfp_cbks->transferPausedByPeer) { sfp_cbks->transferPausedByPeer(event->cid, session->remote_username, session->local_filename, session->file_type, session->file_size); }
		}
	} else if(session->isPaused(session)) {
		session->updateState(session, SFP_ACTION_RESUME);
		if(session->isRunning(session)) {
			if(sfp_cbks != NULL && sfp_cbks->transferResumed) { sfp_cbks->transferResumed(event->cid, session->remote_username, session->local_filename, session->file_type, session->file_size); }
		}
	}
	g_mutex_unlock(pause_mutex);
...
static void sfp_on_EXOSIP_CALL_OFFHOLD(eXosip_event_t * event){
	...
	if(session->isPausedByPeer(session)) {
		...
		if(session->isRunning(session)) {
			if(sfp_cbks != NULL && sfp_cbks->transferResumedByPeer) sfp_cbks->transferResumedByPeer(event->cid, session->remote_username, session->local_filename, session->file_type, session->file_size);
		}
	}
	g_mutex_unlock(pause_mutex);
