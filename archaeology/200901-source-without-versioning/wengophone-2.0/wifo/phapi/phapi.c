...
// mutex for protecting all accesses/modifications on calls
GMutex * ph_media_stop_mutex;
#define PH_MSESSION_START_STOP_LOCK() g_mutex_lock(ph_media_stop_mutex)
#define PH_MSESSION_START_STOP_UNLOCK() g_mutex_unlock(ph_media_stop_mutex)
////
...
void ph_release_call(phcall_t *ca)
{
  DBG_SIP_NEGO("SIP_NEGO: stop call media engine and release call\n");
...
MY_DLLEXPORT int 
phUnsubscribe(int sid)
{
	int i;

	eXosip_lock();
	i = eXosip_subscribe_close(sid);
	eXosip_unlock();  
	return i;
}
...
phConf(int cid1, int cid2)
{
	...
	PH_MSESSION_START_STOP_LOCK();
	ca1 = ph_locate_call_by_cid(cid1);
	ca2 = ph_locate_call_by_cid(cid2);
...
phSendDtmf(int cid, int dtmfEvent, int mode)
{
  ...
  ret = ph_msession_send_dtmf(ca->mses, dtmfEvent, mode);
  PH_MSESSION_START_STOP_UNLOCK();
  return ret;
...
phSendSoundFile(int cid, const char *fileName)
{
  ...
  ret = ph_msession_send_sound_file(ca->mses, fileName);
  PH_MSESSION_START_STOP_UNLOCK();
  return ret;
