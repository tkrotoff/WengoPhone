...
int 
osip_dialog_match_call_id (osip_dialog_t * dialog, const char* callid, const char* to_tag, const char* from_tag)
{
  const char * local = from_tag;
  const char * remote = to_tag;

  if (!dialog->call_id || !dialog->local_tag || !dialog->remote_tag) 
    return 0;

  if (strcmp(callid, dialog->call_id))
    return 0;
  ...
  return (strcmp(dialog->remote_tag, remote) == 0) && ( 0 == strcmp(dialog->local_tag, local));
}
...
osip_dialog_match_with_replaces (osip_dialog_t * dialog, const osip_replaces_t *rep)
{
  ...
  i = osip_dialog_match_call_id(dialog, callid, to, from);
