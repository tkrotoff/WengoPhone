...
public:
	...
	/*LanguageList(const QString & languageListFilename = "lang/lang.txt") {
		QFile file(languageListFilename);
		if (file.open(IO_ReadOnly)) {
			QTextStream stream(&file);
			int i = 1;
			while (!stream.atEnd()) {
				//Line of text excluding '\n'
				QString line(QString::fromUtf8(stream.readLine()));
				_languageList += line;
			}
			file.close();
		}
	...
	LanguageList() {
		const char * pointer;
		int length = sizeof(LANGUAGES) / sizeof(pointer);
		for (int i = 0; i < length; i++) {
			_languageList += QString::fromUtf8(LANGUAGES[i]);
		}
	}

	QString getLanguageName(const QString & iso639Code) {
		if (_languageList.empty()) {
			return QString::null;
		}
		for (QStringList::Iterator it = _languageList.begin(); it != _languageList.end(); ++it) {
			QString line(*it);

			//en... -> en
			int i = line.contains(QRegExp("^" + iso639Code + "\\t"));
			if (i == 1) {
				//en	English... -> English
				QRegExp rx(".*\\t(.*)\\t");
				rx.search(line);
...
/**
 * Language component of the ConfigWindow.
 *
 * Selection of the language for the application.
 *
 * @author Tanguy Krotoff
 */
class LanguageWidget : public QObject {
	Q_OBJECT
public:

	LanguageWidget(QWidget * parent);
...
private:
	...
	LanguageWidget(const LanguageWidget &);
	LanguageWidget & operator=(const LanguageWidget &);

	QStringList getLangNameList();

	static QStringList getLangFileList();


	LanguageList _languageList;

	//       "English", "wengo_en.qm"
	std::map<std::string, std::string> _mapFileLanguage;

	/**
	 * The low-level widget of this gui component.
	 */
	QWidget * _languageWidget;
};
