...
int webcamOpen(struct webcam *w, int width, int height)
{
	...
	capPreviewScale(w->hwndCap, 1);
	capPreview(w->hwndCap, 1);

	capCaptureGetSetup(w->hwndCap, &w->capParams, sizeof(w->capParams));
	w->capParams.fYield = 1;
	w->capParams.fAbortLeftMouse = 0;
	w->capParams.fAbortRightMouse = 0;
	w->capParams.fLimitEnabled = 0;
	w->capParams.fCaptureAudio = 0;
