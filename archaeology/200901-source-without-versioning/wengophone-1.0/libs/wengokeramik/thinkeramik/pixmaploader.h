...
// $Id: pixmaploader.h,v 1.24 2003/09/06 21:17:04 orlovich Exp $
...
namespace ThinKeramik
{
	...
	private:
		...
		struct ThinKeramikCacheEntry
		{
			...
			ThinKeramikCacheEntry(int id, const QColor& color, const QColor& bg, bool disabled,
			...
			bool operator == (const ThinKeramikCacheEntry& other)
			{
		...
		QIntCache <ThinKeramikCacheEntry>  m_pixmapCache;
	...
	protected:
		...
		the IDs to pixmaps, and thinkeramikrc.h, which provides nice symbolic constants for base IDs.
	...
	{
	public:
		SpinBoxPainter() : TilePainter( thinkeramik_spinbox ) {
			colMde[0] = colMde[2] = Fixed;
			colMde[1] = Scaled;
