...
// $Id: pixmaploader.cpp,v 1.40 2003/09/06 21:17:04 orlovich Exp $
...
{
	ThinKeramikEmbedImage* edata = ThinKeramikGetDbImage(name);
	if (!edata)
		return 0;
		...
		if (blend)
		{
			...
			for (int pos = 0; pos < size; pos+=3)
			{
				...
				if (scale < 128) {
					...
					rr = 255 - ((255 - scale) * (255 - r) >> 7);
					rg = 255 - ((255 - scale) * (255 - g) >> 7);
					rb = 255 - ((255 - scale) * (255 - b) >> 7);
				}
				/*
				if (scale != 0)
					add = add*5/4;
...
{
	ThinKeramikCacheEntry entry(name, color, bg, disabled, blend, width, height);
	ThinKeramikCacheEntry* cacheEntry;
...
{
	ThinKeramikEmbedImage* edata = ThinKeramikGetDbImage(id);
	if (!edata)
		return QSize(0,0);
...
ActiveTabPainter::ActiveTabPainter( bool bottom )
	: RectTilePainter( bottom? thinkeramik_tab_bottom_active: thinkeramik_tab_top_active, false),
	  m_bottom( bottom )
{
	m_rows = 2;
	if (m_bottom)
...
InactiveTabPainter::InactiveTabPainter( Mode mode, bool bottom )
	: RectTilePainter( bottom? thinkeramik_tab_bottom_inactive: thinkeramik_tab_top_inactive, false),
...
int InactiveTabPainter::tileName( unsigned int column, unsigned int row ) const
{
	...
	if ( column == 0 && m_mode != check ) return ThinKeramikTileSeparator;
	if ( m_bottom )
		return RectTilePainter::tileName( column, row + 1 );
	return RectTilePainter::tileName( column, row );
}
...
{
	return horizontal? thinkeramik_scrollbar_hbar: thinkeramik_scrollbar_vbar;
}
