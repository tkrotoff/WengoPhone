...
*/
// $Id: gradients.cpp,v 1.7 2003/06/22 02:28:24 orlovich Exp $
...
namespace
{
	struct GradientCacheEntry
	{
		...
		GradientCacheEntry(int width, int height, const QColor& color, bool menu, bool strong3d):
			m_pixmap(0), m_color(color.rgb()), m_menu(menu), m_strong3d(strong3d), m_width(width), m_height(height)
		{}
		
		int key()
		{
			return m_menu ^ m_strong3d ^ m_width ^ (m_height << 16) ^ ((m_color)<<8);
		}
										...
										int pwidth, int pheight)
{
	...
	GradientCacheEntry entry (width, height, c, menu, strong3d);
...
void GradientPainter::renderGradient2( QPainter* p, const QRect& r, QColor c, bool horizontal, bool menu, bool strong3d,
                                        int px, int py, int pwidth, int pheight)
{
	...
	if (horizontal)
	{
		...
		else
		{
			...
			int h2 = height - h1;
                       ...
                       QImage top, bot;
			if (strong3d) {
			      top = KImageEffect::gradient(QSize(4,h1),  ColorUtil::lighten(c,125), c.light(90), KImageEffect::VerticalGradient );
			      bot = KImageEffect::gradient(QSize(4,h2), c.light(90), ColorUtil::lighten(c,124), KImageEffect::VerticalGradient );
			} else {
			      top = KImageEffect::gradient(QSize(4,h1),  ColorUtil::lighten(c,110), c.light(94), KImageEffect::VerticalGradient );
			      bot = KImageEffect::gradient(QSize(4,h2), c.light(94), ColorUtil::lighten(c,109), KImageEffect::VerticalGradient );
			}
	...
	else
	{
		...
		int h2 = width - h1;
               ...
               QImage top, bot;
		if (strong3d) {
		      top = KImageEffect::gradient(QSize(h1,4), ColorUtil::lighten(c,115), c.light(95), KImageEffect::HorizontalGradient );
		      bot = KImageEffect::gradient(QSize(h2,4), c.light(95), ColorUtil::lighten(c,75), KImageEffect::HorizontalGradient );
		} else {
		      top = KImageEffect::gradient(QSize(h1,4), ColorUtil::lighten(c,110), c.light(95), KImageEffect::HorizontalGradient );
		      bot = KImageEffect::gradient(QSize(h2,4), c.light(95), ColorUtil::lighten(c,80), KImageEffect::HorizontalGradient );
		}
