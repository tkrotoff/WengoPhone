...
void TransparencyHandler::rightShadow(QImage& dst)
{
...
#endif
	for(register int i1 = 0; i1 < 16; i1++) {
		*data++ = (unsigned char)((*data)*top_right_corner[i1]);
		*data++ = (unsigned char)((*data)*top_right_corner[i1]);
		*data++ = (unsigned char)((*data)*top_right_corner[i1]);
		data++;	// skip alpha
	}
	...
	register int c = 0;
	for(register int i2 = 0; i2 < pixels; i2++) {
	...
	// tint bottom edge
	for(register int i3 = 0; i3 < 16; i3++) {
		*data++ = (unsigned char)((*data)*bottom_right_corner[i3]);
		*data++ = (unsigned char)((*data)*bottom_right_corner[i3]);
		*data++ = (unsigned char)((*data)*bottom_right_corner[i3]);
		data++;	// skip alpha
	}
...
void TransparencyHandler::bottomShadow(QImage& dst)
{
	...
	for(int y = 0; y < 4; y++)
	{
		...
		// Scanline
		for(register int x2 = 0; x2 < width; x2++) {
