...
struct bwcontrol_stat {
	...
	int rate;
	guint64 drop_count;
	guint64 unavail_count;
	guint64 outoftime_count;
};
...
void ph_video_rtcp_rr_sent(RtpSession *session, rtcp_t * r) {
	...
	for (ca = ph_calls; ca < &ph_calls[PH_MAX_CALLS]; ca++) {
		...
		s = ca->ph_video_stream;
		if (!s) {
			continue;
		}
		if (s->ms.rtp_session == session) {
...
void ph_video_bwcontrol_apply_user_params(phvstream_t *stream) {
	...
	switch (cfg->video_config.video_line_configuration) {
...
ph_video_bwcontrol_thread(void *p)
{
	...
	st = &stream->ms.rtp_session->stats;
	enc_ctx = (ph_h263_encoder_ctx_t *) stream->ms.encoder_ctx;
	av_enc_t = &enc_ctx->encoder_ctx;
