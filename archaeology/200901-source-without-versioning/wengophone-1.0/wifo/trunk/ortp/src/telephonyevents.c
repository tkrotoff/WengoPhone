...
#ifdef MULTI_SSRC
gint rtp_source_telephone_events_supported(RtpSource *src)
{
	if (src->telephone_events_pt==-1)
	{
		/* search for a telephony event payload in the current profile */
		src->telephone_events_pt=rtp_profile_get_payload_number_from_mime(src->profile,"telephone-event");
	}
	return src->telephone_events_pt;
}
#endif
...
#ifdef MULTI_SSRC
mblk_t	*rtp_source_create_telephone_event_packet(RtpSource *src, int start)
{
	mblk_t *mp;
	rtp_header_t *rtp;

	g_return_val_if_fail(src->telephone_events_pt!=-1,NULL);
	...
	/*set the payload type */
	rtp->paytype=src->telephone_events_pt;
 ...
 *	rtp_session/source_create_telephone_event_packet().
 *
...
#ifdef MULTI_SSRC
gint rtp_source_send_dtmf2(RtpSource *src, gchar dtmf, guint32 userts, int duration)
{
	mblk_t *m1,*m2,*m3;
	int tev_type;
	int durationtier = duration/3;
	...
	m1=rtp_source_create_telephone_event_packet(src,1);
	if (m1==NULL) return -1;
	rtp_source_add_telephone_event(src,m1,tev_type,0,0,durationtier);
	/* create a second packet */
	m2=rtp_source_create_telephone_event_packet(src,0);
	if (m2==NULL) return -1;
	rtp_source_add_telephone_event(src,m2,tev_type,0,0,durationtier+durationtier);

	/* create a third and final packet */
	m3=rtp_source_create_telephone_event_packet(src,0);
	if (m3==NULL) return -1;
	rtp_source_add_telephone_event(src,m3,tev_type,1,0,duration);

	/* and now sends them */
	rtp_source_sendm_with_ts(src,m1,userts);
	rtp_source_sendm_with_ts(src,m2,userts+durationtier);
	/* the last packet is sent three times in order to improve reliability*/
	m1=copymsg(m3);
	m2=copymsg(m3);
	/*			NOTE:			*/
	...
	Using iomapped socket, it is possible to avoid the user to kernel copy.
	*/
	rtp_source_sendm_with_ts(src,m3,userts+durationtier+durationtier);
	rtp_source_sendm_with_ts(src,m1,userts+durationtier+durationtier);
	rtp_source_sendm_with_ts(src,m2,userts+durationtier+durationtier);
	return 0;
}
#endif
/**
...
#ifdef MULTI_SSRC
gint rtp_source_read_telephone_event(RtpSource *src,
									  mblk_t *packet,telephone_event_t **tab)
{
	int datasize;
	gint num;
	...
	rtp_header_t *hdr=(rtp_header_t*)packet->b_rptr;
	g_return_val_if_fail(packet->b_cont!=NULL,-1);
	if (hdr->paytype!=src->telephone_events_pt) return 0;  /* this is not tel ev.*/
	datasize=msgdsize(packet);
	tev=*tab=(telephone_event_t*)packet->b_cont->b_rptr;
	/* convert from network to host order what should be */
	num=datasize/sizeof(telephone_event_t);
...
#ifdef MULTI_SSRC
/* for high level telephony event callback */
void rtp_source_check_telephone_events(RtpSession *src, mblk_t *m0)
{
	telephone_event_t *events,*evbuf;
	int num;
	int i;
	...
	{
		/* this is a start of new events. Store the event buffer for later use*/
		if (src->current_tev!=NULL) {
			freemsg(src->current_tev);
			src->current_tev=NULL;
		}
		src->current_tev=copymsg(m0);
		return;
	}
	/* this is a not a begin of event, so update the current event buffer */
	num=(mp->b_wptr-mp->b_rptr)/sizeof(telephone_event_t);
	events=(telephone_event_t*)mp->b_rptr;
	cur_tev=src->current_tev;
	if (cur_tev!=NULL)
	{
		/* first compare timestamp, they must be identical */
		if (((rtp_header_t*)cur_tev->b_rptr)->timestamp==
			...
			for (i=0;i<num;i++)
			{
				if (events[i].E==1)
				{
					...
					if (evbuf[i].E==0){
						evbuf[i].E=1;
						/* this is a end of event, report it */
						rtp_signal_table_emit2(&src->on_telephone_event,(gpointer)(gint)events[i].event);
					}
				}
			}
		}
