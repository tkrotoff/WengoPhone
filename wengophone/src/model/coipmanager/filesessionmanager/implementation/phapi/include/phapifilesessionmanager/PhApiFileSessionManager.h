...
#include <filesessionmanager/IFileSessionManager.h>
...
*/
class PhApiFileSessionManager : public IFileSessionManager {

public :

	PhApiFileSessionManager(UserProfile & userProfile);

	virtual ~PhApiFileSessionManager();

	bool isProtocolSupported(EnumIMProtocol::IMProtocol protocol) const;

	ISendFileSession * createSendFileSession();
