...
void QtTrayIcon::setTrayMenu() {
	...
	// Change status
	if (_qtWengoPhone->getCWengoPhone().getCUserProfile()) {
...
QMenu * QtTrayIcon::createStatusMenu() {
   ...
   QtProfileBar * profileBar;

   switch ( cWengoPhone.getCUserProfile()->getUserProfile().getPresenceState() ){
    ...
    profileBar = _qtWengoPhone->getProfileBar();
...
void QtTrayIcon::updateCallMenu() {
	...
	CUserProfile *currentCUserProfile = cWengoPhone.getCUserProfile();
